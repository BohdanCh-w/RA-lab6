...
class CriteriasManager:
    ...
    def analise_and_count(self):
        ...
        for crt in self.crts:
            lst = []
            for key in self.exp_weight.keys():
                crt.rate[key].val = crt.rate[key].mark * crt.rate[key].weight
                lst.append(crt.rate[key].val * self.exp_weight[key])
            crt.rate['avrg'] = sum(lst) / sum(self.exp_weight.values())
        ...
        for key in ('industry', 'usability', 'programming', 'users'):
            rt.avrg_w[key] = self.exp_weight[key] * rt.avrg[key]
        ...
        rt.avrg_w['avrg'] = values / len(self.exp_weight.keys())
        rt.avrg_w['avrg_x'] = values_w / sum(self.exp_weight.values())
